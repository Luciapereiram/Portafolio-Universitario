...
if __name__ == "__main__":
	...
	while True:
		try:
			msg = input('Introduce la dirección IP a resolver (q para salir, p para mostrar la caché y g para sondeo ARP):\n')
			if msg == 'q':
				break
			elif msg == 'p':
				printCache()
				...
				try:
					msg = input('Direccion IP a comprobar si es unica en la red:\n')
					ip=struct.unpack('!I',socket.inet_aton(msg))[0]
					ret = ARPResolution(ip, True)
					if ret is not None:
						print('--> Direccion IP ya asignada en la red')
					else:
						print('--> Direccion IP NO asignada en la red')
				except OSError:
